...
def get_options(args=None):
    ...
    optParser.add_argument("-CAVRe", "--CAV.rerouting", dest="CAVRe",
                           default="true", help="the vehicle has the rerouting device or not")                       
...
def CAVFileGenerator(options):
    with open(options.CAVf, 'w') as CAVf:
        ...
        CAVf.write('    <vType id="CAV" carFollowModel="Krauss" accel="3.5" sigma="0.0" tau="0.9" minGap="1.5" laneChangeModel="LC2013" lcAssertive="0.7" color="0,255,255">\n')
        if options.CAVRe=="false":
            CAVf.write('        <param key="has.rerouting.device" value="%s"/>\n' % (options.CAVRe))
        if options.CAVRe=="true":
...
def HDVFileGenerator(options):
    with open(options.HDVf, 'w') as HDVf:
        ...
        HDVf.write('    <vType id="HDV" carFollowModel="Krauss" accel="2.6" sigma="0.5" tau="1.0" minGap="2.5" laneChangeModel="LC2013" lcAssertive="1.3" color="1,0,0">\n')
        if options.HDVRe=="false":
            HDVf.write('        <param key="has.rerouting.device" value="%s"/>\n' % (options.HDVRe))
        if options.HDVRe=="true":
