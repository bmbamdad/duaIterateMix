...
"""
Run duarouter and sumo alternating to perform a mix traffic assignment 
"""
...
def calcMixTrips(step, options):
    src_tree = ET.parse(os.path.join("%03i" % step, ("trips.trips.CAV_%03i.rou.xml" % step)))
    dest_tree = ET.parse(os.path.join("%03i" % step, ("trips_%03i.rou.xml" % step)))
    ...
    ET.ElementTree(src_root).write(os.path.join("%03i" % step, ("trips_%03i.rou.xml" % step)))
    if DEBUGLOG:
        log.close()
def main(args=None):
...
# first you should import non CAVs demand
        ...
        if not (options.skipFirstRouting and step == options.firstStep):
            ...
            for router_input, output in zip(router_demands, simulation_demands):
                ...
                log.flush()
                sys.stdout.flush()
                #removing marginal cost calculation for Non CAVS
